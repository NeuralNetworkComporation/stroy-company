...
(() => {
  ...
  /* Video sits in the site's own player; nothing is fetched until play. */
  const wireVideos = () => {
    window.BH_PLAYER?.init(modalBody);
  };

  const closeModal = () => {
    closeLightbox();
    // Dropping the source stops playback instead of leaving audio running behind the page.
    window.BH_PLAYER?.stopAll(modalBody);
